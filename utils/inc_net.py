...
from convs.recorder import EmbeddingRecorder
...
class IncrementalNet(BaseNet):
    def __init__(self, args, pretrained, gradcam=False, record_embedding: bool = False):
    ...
    def forward(self, x):
        x = self.convnet(x)
        #x = self.embedding_recorder(x) #Normally this line does not exist
        out = self.fc(x["features"])
        out.update(x)
        out = self.embedding_recorder(out)
        if hasattr(self, "gradcam") and self.gradcam:
